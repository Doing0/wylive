...
class WyLiveApi {
    ...
    {
        $this->AppKey = WY_APPKEY;
        $this->AppSecret = WY_APPSCECRET;
        $this->RequestType = WY_REQUESTTYPE;
    }
...
//        print_r($http_header);die;
     ...
     * @param  $type [0 表示文本消息,1 表示图片，2 表示语音，3 表示视频，4 表示地理位置信息，6 表示文件，100
     *     自定义消息类型]
     ...
     * @param  $pushcontent [推送内容，发送消息（文本消息除外，type=0），option选项中允许推送（push=true），此字段可以指定推送内容。
     *     最长200字节]
     *
     * @return $result      [返回array数组对象]
     */
     ...
     * @param  $pushcontent [ios推送内容，第三方自己组装的推送内容，如果此属性为空串，自定义通知将不会有推送（pushcontent
     *     + payload不能超过200字节）]
     * @param  $payload [ios 推送对应的payload,必须是JSON（pushcontent +
     *     payload不能超过200字节）]
